...
{
    using OrderTuple = std::tuple<std::string, std::string, std::string, QtyT, PriceT, TimeT>;
    ...
    for (const auto& input : inputs) {
        matchingEngine.addOrder(std::get<0>(input), std::get<1>(input), std::get<2>(input),
                                std::get<3>(input), std::get<4>(input), std::get<5>(input));
    }
