...
bool validateInput(const std::string& orderId, const std::string& side, 
    ...
    if (qtyStr.empty()) {
        ...
        if (qtyStr[0] == '-') {
            std::cerr << "Quantity should not be a negative number!" << std::endl;
            return false;
        }

        if (auto [ptr, ec] = std::from_chars(qtyStr.c_str(), qtyStr.c_str() + qtyStr.size(), qty); ec != std::errc()) {
            std::cerr << "Unable to parse Quantity! ERROR: " << std::make_error_code(ec).message() << std::endl;
            return false;
        }
    ...
    if (prcStr.empty()) {
        ...
        if (auto [ptr, ec] = std::from_chars(prcStr.c_str(), prcStr.c_str() + prcStr.size(), prc); ec != std::errc()) {
            std::cerr << "Unable to parse Price! ERROR: " << std::make_error_code(ec).message() << std::endl;
            return false;
        }
...
int main() {
    ...
    while (getline(std::cin, line) && line != "exit") {
        ...
        gemini::QtyT qty{};
        gemini::PriceT prc{};
        if (not validateInput(orderId, side, symbol, qtyStr, prcStr, qty, prc)) {
