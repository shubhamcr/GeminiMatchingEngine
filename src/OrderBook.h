...
public:
    ...
    bool isLimitOrder(const Order& order) const noexcept { return order.m_prc > 0; }
    bool isBuyOrder(const Order& order) const noexcept { return order.m_side == Side::BUY; }
    bool isSellOrder(const Order& order) const noexcept { return order.m_side == Side::SELL; }
